...
One accession (MP1) is excluded by the script: read counts of its two
technical-replicate libraries showed the reported genotype came from a library
contaminated during preparation (see the Methods of the manuscript).
...
1. Read the DArT SNP report + metadata into a `genlight` object; populations =
   the 9 geographic **sampling locations** (Figure 1); exclude MP1 and the
   1,021 loci polymorphic only in MP1
2. Quality-filter the SNPs (call rate ≥ 80%, read depth 5–1000, reproducibility ≥ 99%)
   → **119 individuals × 4,790 SNPs**, and descriptive statistics of the retained
...
7. Population structure with fastSTRUCTURE, K = 1–10 with ten seeded replicates
   ...
   components used (**K = 4**; Figures 4 and 5); location × cluster table and
   AMOVA by cluster
8. AMOVA among / within locations, 9,999 permutations
9. Core subset: top 20 genotypes on each of four diversity metrics (48 unique),
   minus 7 near-identical replicates (> 98% identical calls) → **41 genotypes**;
...
fastSTRUCTURE runs; runs already present in `outputs/faststructure/` are
reused, so a rerun takes about 10 minutes. Tables (CSV) go to `outputs/`, figures to `figures/`, and the
